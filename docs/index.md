...
It is assumed that the character are always moving about their area so moving is only required to remove conditions (stand up, clear head), interact with particular items or other characters (reload a weapon, drag a wounded ally, push through) or approach targets in different areas. To move from one zone to another, an unblocked path (may interactions) and full movement (-2 move penalty to all actions) is required.  <sub><sup>[(Table of contents)](#table-of-contents)</sup></sub> 
