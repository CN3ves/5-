...
During each resting time, the player may choose to take time to practice some descriptors. If so, then for each practice the player chooses two marked descriptors and removed the marking in order to attempt a free level up 2d6 roll for one of them as focusing the practice on one skill requires another to be left aside. * <span style="color:grey">Note that, even though the default level up system is random, players may decide to provide buy points after certain milestones to reward their characters in more direct ways.</span>* <sub><sup>[(Back to start)](#)</sup></sub>   
...
It is assumed that the character are always moving about their area so moving is only required to remove conditions (stand up, clear head), interact with particular items or other characters (reload a weapon, drag a wounded ally, push through) or approach targets in different areas. To move from one zone to another, an unblocked path (may interactions) and full movement (-2 move penalty to all actions) is required. <sub><sup>[(Back to start)](#)</sup></sub>   
...
* When gathering information in towns, taverns or any other suitable place, allow the players to add their own rumors, hints and clues. These may be interesting twists to the story, suggestion  of side quests to take or simple adding background history to make the city feel more real. <sub><sup>[(Back to start)](#)</sup></sub>   
...
Once a card is played, the challenge is introduced by the player that used the card, who is free add lore, locations and characters to the game, or even use previously mentioned characters or location and integrate them into their own scenes. <sub><sup>[(Back to start)](#)</sup></sub>  
