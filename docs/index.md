...
**Upgrading**: While the skill level represents how successful the character is using the skill, the skill tier reflects the power, specificity, methodology and deeper understanding of the skill. Higher power and more advanced methods are more difficult to handle. When upgrading a skill tier, you gain:  
  ...
  2. A new [ability](#15-character-abilities) at tier 0 and level 0 attached to your skill. These abilities should provide special effects on top of the base skill.   
...
Besides deciding on what actions to do, players may also decide to call on an effect. Effects arise from any type of descriptor that te character may have, anything from hidden to bleeding. Calling on an effect generally allows players to make tradeoffs, temporally removing the default effect. For example, consider how you can:
